...
impl DF {
    ...
    fn analysis<'py>(
            ...
            input.as_array().axis_iter(Axis(0)).zip(output.axis_iter_mut(Axis(0)))
        {
            ...
            for (ichunk, ochunk) in in_chunks.into_iter().zip(out_chunks) {
...
fn libdf(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<DF>()?;
    m.add_function(wrap_pyfunction!(erb, m)?)?;
    m.add_function(wrap_pyfunction!(erb_inv, m)?)?;
    m.add_function(wrap_pyfunction!(erb_norm, m)?)?;
    m.add_function(wrap_pyfunction!(unit_norm, m)?)?;
    m.add_function(wrap_pyfunction!(unit_norm_init, m)?)?;
